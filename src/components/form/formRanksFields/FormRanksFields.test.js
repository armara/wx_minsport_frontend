...
import Ranks from 'components/form/formRanksFields/FormRanksFields'
...
describe('<Ranks />', () => {
  ...
  it('Should render ranks from props', () => {
    const wrapper = mount(component)
    const content = wrapper.find('.dropdown-content')
    ...
    expect(pos1.text()).toBe('Без звания')
    expect(pos2.text()).toBe('Третий юношеский спортивный разряд')
  })
})
